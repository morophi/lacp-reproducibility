...
SET @run_id = 'tr_preflight_runb_smoke_ragok_001';
...
SELECT run_id, turn_no, node, endpoint_mode, rag_injected, sc_policy_applied,
       ...
       raw_logprobs_len, clean_logprobs_len,
       JSON_LENGTH(rag_chunk_ids) AS rag_chunk_count,
...
SELECT run_id, turn_no, node, rag_injected, sc_policy_applied, trigger_mode,
       trigger_reasons, JSON_LENGTH(rag_chunk_ids) AS rag_chunk_count, rag_chunk_ids
FROM v_intervention_logs
WHERE run_id = @run_id
ORDER BY turn_no, node;
