...
# v10.4 Addendum: Thermal Control Checklist for Rev8 RUN Stages

> Paper alignment: `experiment/lacp_ijibc_rev8.docx`
> Guideline alignment: `experiment/lacp_experiment_guideline_rev5.5.md`
> Date: 2026-05-26

## Global Thermal Policy

```text
□ Treat full-speed fan operation as the default state for TR/CR/CR2/CF/CF-F.
□ Do not reduce fan speed during formal RUN stages unless a prior thermal-only
  validation proves equivalent peak temperature and cooldown recovery.
□ Keep the same thermal policy across all experimental conditions.
□ Record any deviation from full-speed fan operation as an operational exception.
```

## Inference Node Temperature Source

Inference nodes use Fedora. Preferred temperature source:

```text
/sys/class/hwmon/hwmon*/temp*_input
```

Required checks per inference node:

```text
□ inference1 hwmon temperature readable
□ inference2 hwmon temperature readable
□ inference3 hwmon temperature readable
□ nvme temperature label visible when available
□ amdgpu edge temperature visible when available
□ k10temp Tctl temperature visible when available
□ fallback thermal_zone path checked only when hwmon is unavailable
```

## Pre-Run Fan and Thermal Readiness

Before starting TR or any formal RUN stage:

```text
□ inference1 powered on and SSH reachable
□ inference2 powered on and SSH reachable
□ inference3 powered on and SSH reachable
□ Ollama port 11434 reachable on inference1/2/3
□ fan controller powered and configured
□ fan speed set to full-speed or approved rev8 fan curve
□ room/door/ventilation state recorded if relevant
□ idle temperature snapshot recorded for inference1/2/3
□ idle max temperature reviewed before dispatch
```

## Turn-Level Barrier Verification

```text
□ Harness dispatches the same turn to A/B/C.
□ Harness waits for all A/B/C responses before the next turn.
□ nodes_completed is returned only after all three nodes finish.
□ No node starts turn N+1 before all nodes finish turn N.
□ Per-node elapsed_ms is logged as diagnostic metadata.
```

## Five-Turn Cooldown Verification

At turns 5, 10, 15, 20, 25, and 30:

```text
□ pre-cooldown temperature snapshot recorded
□ cooldown_start event recorded
□ 30-second cooldown completed
□ cooldown_end event recorded
□ post-cooldown temperature snapshot recorded
□ next batch starts only after cooldown_end
```

## Post-Run Thermal Review

After the RUN:

```text
□ final response completion temperature reviewed
□ cooldown-end temperature reviewed
□ peak temperature per node summarized
□ start-to-peak rise summarized
□ cooldown-end drop summarized
□ node dropout / throttling / emergency shutdown checked
□ thermal log archived with run_id
```

## Current Thermal-Only Reference Snapshot
...
Observed max temperatures:

```text
inference1: initial 48.375 C, peak 74.0 C, 10-sec cooldown 67.5 C
inference2: initial 49.5 C,   peak 69.0 C, 10-sec cooldown 64.0 C
inference3: initial 49.375 C, peak 74.0 C, 10-sec cooldown 67.375 C
```

Interpretation:

```text
□ Full-speed fan operation is a minimum defensive condition, not excess cooling.
□ Long 30-turn stages may approach higher sustained temperatures than the
  five-turn probe.
□ Thermal cooldown is required for node stability and data persistence.
```

## v10.5 Parallel 60-Second Cooldown Reference

Use the 60-second parallel thermal probe as the preferred reference for fan and
cooldown readiness checks.

```text
run_id = thermal_only_5turn_cooldown60_parallel_20260526T170010
sampling = parallel inference-node sampling
average_sample_interval = approximately 1.004 sec
turns = 5
cooldown_observation = 60 sec
inference_failures = 0
```

Reference cooldown behavior:

```text
inference1: 72.0 C -> 51.875 C over 60 sec
inference2: 69.0 C -> 52.25 C over 60 sec
inference3: 73.0 C -> 52.125 C over 60 sec
```

Cooling shape check:

```text
□ Cooling is expected to be early-fast and late-slow, not linear.
□ First cooldown segment slope should be steeper than late segment slope.
□ If late segment remains steep or temperature does not drop, inspect room
  ventilation, fan speed, and exhaust path before formal RUN.
□ If peak exceeds the previous reference by a large margin, pause formal RUN
  and repeat thermal-only validation.
```

Pre-formal RUN thermal acceptance:

```text
□ Full-speed fan or approved fan curve active.
□ Idle max temperature is near the known baseline range.
□ Five-turn thermal-only probe does not exceed acceptable peak range.
□ 30-second cooldown produces a clear temperature drop.
□ 60-second reference remains available for post-run comparison.
```
