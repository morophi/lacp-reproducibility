...
## F. Rev3.1 Parallel Thermal Sampling Requirement

The thermal logger MUST sample inference nodes in parallel when used to justify
cooldown policy or to produce formal run-control evidence.

Rationale:

```text
Sequential SSH sampling caused node-level intervals to drift above the intended
1-second term. Parallel sampling restored the observed average sample interval
to approximately 1.004 sec across inference1/2/3.
```
...
```text
□ Thermal snapshots for A/B/C infrastructure should be collected concurrently.
□ cooldown_start and cooldown_end should include the same batch_end_turn.
□ temp_before and temp_after should carry node-keyed values for inference1/2/3.
□ Thermal event rows must be excluded from metric completeness calculations.
□ Cooling should not be modeled as a linear function in formal interpretation.
```

Recommended event payload extension:
...
```json
{
  ...
  "cooling_interpretation": "nonlinear_early_fast_late_slow",
  "temp_before": {
    "inference1": {"max_temp_c": 72.0},
    "inference2": {"max_temp_c": 69.0},
    "inference3": {"max_temp_c": 73.0}
  },
  "temp_after": {
    "inference1": {"max_temp_c": 51.875},
    "inference2": {"max_temp_c": 52.25},
    "inference3": {"max_temp_c": 52.125}
  }
}
```
