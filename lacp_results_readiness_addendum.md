...
The experiment must validate corpus quality before any cross-node comparison. The Pure RAG Ingest Test Run uses no LLM inference nodes and exists only to verify the corpus before Node A/B/C comparison.
...
??CR entry is blocked until Pure RAG Ingest Test Run and Retrieval Coherence Test pass

??RAG corpus defects are resolved before CR, Run B, or CF outputs can be interpreted
```

---

## 2. Pure RAG Ingest Test Run Go / No-Go

Purpose: verify PDF/HWPX ingestion and retrieval quality before corpus errors can contaminate CR, Run B, or CF results.

This test run must not use LLM inference nodes. It validates the corpus itself before Node A/B/C comparison.

```
??canonical.md generation completed
  Output file: canonical_sample_review.md

??source page / section / table metadata is preserved
  Manifest file: chunk_manifest.jsonl

??eligibility / exception / conditional marker tagging is verified
  Review file: canonical_sample_review.md

??target-condition scope and exclusion-condition scope are not mixed within chunks
  Evidence file: retrieval_coherence_report.md

??table-based selection criteria preserve meaning after flattening
  Accepted structures: markdown table or key-value structure
  Review file: canonical_sample_review.md

??chunk boundary coherence is verified
  Manifest file: chunk_manifest.jsonl

??corpus hash and chunk hash are recorded
  Output file: corpus_hash.txt

??embedding manifest and embedding hash are recorded
  Manifest file: embedding_manifest.json
  Hash file: embedding_hash.txt

??Any failed ingest cases are documented
  Failure file: failed_cases.md

??CR entry decision
  ??GO
  ??NO-GO
  Rule: if any item above fails, CR must not start.
```
...
## 3. Retrieval Coherence Test Go / No-Go

Purpose: verify that embedding and retrieval preserve source policy meaning before downstream experiment runs.

```
??Representative query set is fixed
  Query file: retrieval_test_queries.json

??For representative queries, top-k retrieval results match source policy meaning
  Report file: retrieval_coherence_report.md

??For exception queries, exclusion-condition chunks appear within top-k
  Report file: retrieval_coherence_report.md

??Repeated identical queries produce stable retrieval ranking
  Stability evidence file: retrieval_coherence_report.md
...
??CR entry decision
  ??GO
  ??NO-GO
  Rule: if any retrieval coherence item fails, CR must not start.
```
...
## 4. Manuscript-Checklist Consistency
...
## 5. Node Role and Treatment Integrity
...
## 6. Simultaneity and Runtime Variance
...
## 7. LMS and Entropy Threshold Readiness
...
## 12. Counterfactual Condition Readiness
...
## 14. Database Sanity Queries Before Analysis
...
```
□ Experiment execution order includes Pure RAG Ingest Test Run and Retrieval Coherence Test before CR
